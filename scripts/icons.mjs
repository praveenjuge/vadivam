...
const graphics = new Set(["path", "circle", "line", "polyline", "polygon", "rect", "ellipse"]);
const geometryAttrs = {
  path: new Set(["@_d"]),
...
async function svgFiles() {
  return (await readdir(iconsDir)).filter((file) => file.endsWith(".svg")).sort();
}
...
function parseSvg(svg, file = "inline.svg") {
  ...
  assert(parsed.svg && typeof parsed.svg === "object", `${file}: missing <svg> root`);
  return parsed.svg;
}
...
export function validateSvgContent(svg, file = "inline.svg") {
  ...
  for (const key of Object.keys(node)) {
    if (key.startsWith("@_")) assert(key in rootAttrs, `${file}: unsupported root attribute ${key.slice(2)}`);
  }
  ...
  const visit = (current, parentTag) => {
    for (const [tag, value] of Object.entries(current)) {
      ...
      for (const item of asArray(value)) {
        count += 1;
        assert(item && typeof item === "object", `${file}: invalid <${tag}> element`);
        const allowed = geometryAttrs[tag];
        for (const attr of Object.keys(item)) {
          if (attr.startsWith("@_")) {
            assert(allowed.has(attr), `${file}: unsupported ${tag} attribute ${attr.slice(2)}`);
          } else {
            visit({ [attr]: item[attr] }, tag);
          }
        }
        assert(Object.keys(item).some((attr) => allowed.has(attr)), `${file}: <${tag}> has no geometry`);
      }
    }
  };
...
async function buildRawPackage(icons) {
  ...
  for (const icon of icons) {
    await copyFile(path.join(iconsDir, icon.fileName), path.join(rawDist, "icons", icon.fileName));
  }
  ...
  await writeFile(path.join(rawDist, "index.js"), `export { icons, iconNames, iconsByName } from "./manifest.js";\n`);
  await writeFile(path.join(rawDist, "index.d.ts"), `export { icons, iconNames, iconsByName } from "./manifest.js";\nexport type { IconNode, VadivamIconMetadata } from "./manifest.js";\n`);
}
...
async function buildReactPackage(icons) {
  ...
  await writeFile(path.join(reactDist, "createIcon.d.ts"), `import type { IconNode, VadivamIcon } from "./types.js";\nexport declare function createIcon(displayName: string, iconNode: IconNode): VadivamIcon;\n`);
  ...
  const namedExports = icons.map((icon) => `export { default as ${icon.componentName} } from "./icons/${icon.name}.js";`).join("\n");
  const dynamicEntries = icons.map((icon) => `  "${icon.name}": () => import("./icons/${icon.name}.js")`).join(",\n");
  const dynamicTypes = icons.map((icon) => `  readonly "${icon.name}": () => Promise<{ default: VadivamIcon }>;`).join("\n");
  await writeFile(path.join(reactDist, "index.js"), `${namedExports}\nexport { createIcon } from "./createIcon.js";\nexport { DynamicIcon } from "./dynamic.js";\nexport { default as dynamicIconImports } from "./dynamicIconImports.js";\n`);
  await writeFile(path.join(reactDist, "index.d.ts"), `${namedExports}\nexport { createIcon } from "./createIcon.js";\nexport { DynamicIcon } from "./dynamic.js";\nexport { default as dynamicIconImports } from "./dynamicIconImports.js";\nexport type { IconNode, VadivamIcon, VadivamProps } from "./types.js";\n`);
  await writeFile(path.join(reactDist, "dynamicIconImports.js"), `const dynamicIconImports = {\n${dynamicEntries}\n};\nexport { dynamicIconImports };\nexport default dynamicIconImports;\n`);
  await writeFile(path.join(reactDist, "dynamicIconImports.d.ts"), `import type { VadivamIcon } from "./types.js";\ndeclare const dynamicIconImports: {\n${dynamicTypes}\n};\nexport { dynamicIconImports };\nexport default dynamicIconImports;\n`);
  await writeFile(
...
async function buildWebAssets(icons) {
  ...
  for (const icon of icons) {
    await copyFile(path.join(iconsDir, icon.fileName), path.join(webIconsDir, icon.fileName));
  }
}
