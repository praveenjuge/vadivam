...
Choose **Next 20** in the **shadcn icons** row to create the next missing batch from the fixed shadcn coverage queue. The queue contains canonical current Lucide names, skips matching icons already present anywhere in the Figma document, and advances by up to 20 frames per click using the same source-frame format and placement rules.
