...
<Steps>
  <Step title="Normalize the source SVGs">
...
```
  ...
  <Step title="Validate without writing further changes">
...
```
  ...
  <Step title="Regenerate derived assets">

```sh
bun run icons:build
```

The generated package and site assets come from this command. Do not hand-edit generated output to fix an icon; correct the canonical SVG and regenerate.
...
Documentation source lives in `apps/docs/docs`. Use Markdown where possible, and MDX only when a page needs built-in components. Include `title` and `description` YAML frontmatter, keep examples executable, and use links rooted at `/docs` for documentation pages.
