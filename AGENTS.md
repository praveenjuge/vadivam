...
- Deploy the static Workers site: `bun run deploy` (docs changelog is fetched from GitHub Releases at build time — set `GITHUB_TOKEN` if the unauthenticated API is rate-limited)
